...
class MaxPool(torch.nn.Module):
    ...
    def forward(
        ...
        logits: torch.Tensor
            (B, OUT_FEATURES)
        """
        max_x, _ = torch.max(x.masked_fill(mask, '-inf'), dim=1)

        return self.mlp(max_x), None


class MinMaxPool(torch.nn.Module):
    """
    MaxPool Module
    Parameters
    ----------
    ...
    def forward(
        ...
        """
        max_x, _ = torch.max(x.masked_fill(mask, '-inf'), dim=1)
        min_x, _ = torch.min(x.masked_fill(mask, '+inf'), dim=1)
        aggregated_feature = torch.cat((min_x, max_x), dim=1)
