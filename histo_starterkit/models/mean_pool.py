...
class MeanVarPool(torch.nn.Module):
    """
    MeanPool Module
    Parameters
    ----------
    ...
    def forward(
        ...
        if mask is not None:
            ...
            # Only var over non padded features
            var_x = torch.sum((x - mean_x).masked_fill(mask, 0.0) ** 2, dim=1) / torch.sum(
                (~mask).float(), dim=1
            )
        else:
            mean_x = torch.mean(x, dim=1)
        ...
        aggregated_feature = torch.cat((mean_x, var_x), dim=1)

        return self.mlp(aggregated_feature), None